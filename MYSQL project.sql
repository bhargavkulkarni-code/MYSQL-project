...
-- write the query to convert ascending order of name from store?
select name from store order by name asc;
-- write the query to fetch userid, name by using group by query from users?
...
-- write the query to fetch brand and max price of brand by using group by query?
select brand,max(price) from product group by brand;
-- write the query to fetch addrid, maximum userid,name,contactphoneNumber,city by using group by query to addrid and convert ascending order to city?
...
-- write the query to fetch name and maximum price from product by using group by name? 
...
-- write the query to fetch name, maximum amount from product by using group by name where amount is greater than or equal to 50?
